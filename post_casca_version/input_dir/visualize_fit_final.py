...
import argparse
import emcee
from model_profiles_pfk import gaussring_model, gaussring_1blob_model, gaussring_2blob_model, model_label
...
def main():
    ...
    if fittype == 'gaussring':
        ...
        logging.info('twodring_2blob_ne')
        ...
        sigma_b1_rad = pars_bf[8] * arcsec
        dist1_rad = pars_bf[9] * arcsec
        ang1_rad = pars_bf[10] * deg

        sigma_b2_rad = pars_bf[12] * arcsec
        dist2_rad = pars_bf[13] * arcsec
        ang2_rad = pars_bf[14] * deg

        model_img = gaussring_2blob_model(pars_bf[0], sigma_rad, ring_rad, inc_rad, pars_bf[7], sigma_b1_rad, dist1_rad, ang1_rad, pars_bf[11], sigma_b2_rad, dist2_rad, ang2_rad, nxy, dxy)
        model = np.array(sampleImage(model_img, dxy, u_datx, v_datx, dRA=dra_rad, dDec=ddec_rad, PA=pa_rad, origin='lower'), dtype=np.complex256)
    ...
    if fittype == 'gaussring':
        ...
        logging.info('twodring_2blob_ne')
        ring_model = make_model_2blob(numpix, pixarcsec, pars_bf[0], pars_bf[1], pars_bf[2], pars_bf[3], pars_bf[4], pars_bf[5], pars_bf[6], pars_bf[7], pars_bf[8], pars_bf[9], pars_bf[10], pars_bf[11], pars_bf[12], pars_bf[13], pars_bf[14])
    ...
    im3 = ax3.imshow(resid_plot, vmin=np.percentile(resid_plot, 1), vmax=np.percentile(data_plot, 99.95), origin='lower', cmap='coolwarm', rasterized=True)
