...
def main():
    ...
    visFile = '/arc/projects/uvdisk_fit/ngc_3351/JS_data/cont93GHz/M95_C5+C2_cont93_uvtable.txt'
    ...
    u_dat, v_dat, Re_dat, Im_dat, w_dat = np.require(np.loadtxt(visFile, unpack=True), requirements='C')
    wavelength = 299792458/93e9
    u_dat /= wavelength
    v_dat /= wavelength
    ...
    comp_plot = filepath+'_compplot.pdf'
    ...
    uvx = UVTable(uvtable=[u_dat*wavelength, v_dat*wavelength, Re_dat, Im_dat, w_dat], wle=wavelength, columns=COLUMNS_V0)
    uvx.apply_phase(-dra_rad, -ddec_rad)         # center the source on the phase center
    uvx.deproject(inc_rad, pa_rad)
    axes = uvx.plot(linestyle='.', color='k', label='Data', uvbin_size=uvbin_size)

    # model uv-plot
    uvx_mod = UVTable(uvtable=[u_dat*wavelength, v_dat*wavelength, model.real, model.imag, w_dat], wle=wavelength, columns=COLUMNS_V0)
    uvx_mod.apply_phase(-dra_rad, -ddec_rad)      # center the source on the phase center
    uvx_mod.deproject(inc_rad, pa_rad)
    uvx_mod.plot(axes=axes, linestyle='-', color='r', label='Model', yerr=False, uvbin_size=uvbin_size)

    #axes2.set_title("YY")

    # observations uv-plot
    #uvy = UVTable(uvtable=[u_daty*wavelength, v_daty*wavelength, Re_daty, Im_daty, w_daty], wle=wavelength)
    #uvy.apply_phase(-dra_rad, -ddec_rad)         # center the source on the phase center
    #uvy.deproject(inc_rad, pa_rad)
    #uvy.plot(axes=axes2, linestyle='.', color='k', label='Data', uvbin_size=uvbin_size)

    # model uv-plot
    #uvy_mod = UVTable(uvtable=[u_daty*wavelength, v_daty*wavelength, model.real, model.imag, w_daty], wle=wavelength)
    #uvy_mod.apply_phase(-dra_rad, -ddec_rad)      # center the source on the phase center
    #uvy_mod.deproject(inc_rad, pa_rad)
    #uvy_mod.plot(axes=axes2, linestyle='-', color='r', label='Model', yerr=False, uvbin_size=uvbin_size)

    #fig.subplots_adjust(hspace=0.1,wspace=0.1)
    uv_plot = filepath+'_uvplot.pdf'
    axes[0].figure.savefig(fname=uv_plot, bbox_inches='tight')
